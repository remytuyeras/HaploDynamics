...
```
 ...
 - **abstract:** This function encodes a [discrete](https://en.wikipedia.org/wiki/Step_function) probability distribution on a set of allele frequency intervals for a given population.
 - **inputs:**
   ...
   - ```alpha```: a floating-point number that measures the proportion of rare mutations in a given population. This number is expected to be higher for populations that have experienced a bottleneck event (e.g. YRI is ```0.17```, CEU is ```0.9``` and CHB+JPT is ```0.8```);
      > **Note**
      > For best practice, use the **first** ouptut of the functions ```population_mut()``` or ```population_mld()``` to generate the value ```alpha```.
      >
 - **output:**
   - a floating-point number representing the probability that an allele frequency is in the interval $[i/15,(i+1)/15]$ for a population characterized by the value ```alpha```.
...
```
 ...
 - **abstract:** This function defines a random generator returning sub-intervals of $[0,1]$ according to the discrete probability distribution ```afs_distribution()```.
 - **inputs:**
...
```
 ...
 - **abstract:**  This function generalizes the random generator ```afs_intervals()```, which returns allele frequency intervals, into a random generator returning allele frequencies.
 - **inputs:** 
   - ```alpha```: a floating-point number that measures the proportion of rare mutations in a given population. This number is expected to be higher for populations that have experienced a bottleneck event (e.g. YRI is ```0.17```, CEU is ```0.9``` and CHB+JPT is ```0.8```);
      > **Note**
      > For best practice, use the **first** ouptut of the functions ```population_mut()``` or ```population_mld()``` to generate the value ```alpha```.
...
```
 ...
 - **abstract:**  The output of this function can be used by other functions of the library to simulate rows of genetic variations at a hypothetical genetic position for a hypothetical population. Specifically, the function returns two elements: a minor allele frequency $p$ and its associated Hardy-Weinberg probability distribution $Q$.
      > **Note**
      > In terms of implementation, the probability distribution $Q$ is encoded as a tiling of the interval $[0,1]$ so that it can be simulated via a [Monte Carlo process](https://en.wikipedia.org/wiki/Monte_Carlo_method).
      >
 - **inputs:**
   - ```alpha```: a floating-point number that measures the proportion of rare mutations in a given population. This number is expected to be higher for populations that have experienced a bottleneck event (e.g. YRI is ```0.17```, CEU is ```0.9``` and CHB+JPT is ```0.8```);
      > **Note**
...
```
 ...
 - **inputs:**
   - ```hwp```: a list of five floating-point numbers defining a tiling of the interval $[0,1]$. This tiling is used with a [Monte Carlo process](https://en.wikipedia.org/wiki/Monte_Carlo_method) to simulate a Hardy-Weinberg probability distribution.
      > **Note**
      > For best practice, use the second outputs of the functions ```genotype_schema()``` or ```cond_genotype_schema()``` to generate the list ```hwp```;
      >
   - ```minor ```: an integer value from the set $\lbrace 0,2\rbrace$ to indicate whether the minor allele of the hypothetical genotype corresponds to the nucleotide of a hypothetical reference genome sequence. 
      > **Note**
      > For best practice, ue the value $0$ to simulate a situation where the minor allele corresponds to the nucleotide of the reference genome sequence.
      >
 - **output:**
...
```
 ...
 - **inputs:**
   - ```beta```: an amplitude $\beta$ for the decay behavior of the standard LD measure $r^2$. 
      > **Note**
      > For best practice, use the **second** ouptut of the function ```population_mld()``` to generate the value ```beta```.
      >
...
```
 ...
 - **inputs:**
   - ```alpha```: a bottleneck parameter $\alpha$ that characterizes the amount of rare mutations in a population (e.g. YRI is ```0.17```, CEU is ```0.9``` and CHB+JPT is ```0.8```);
      > **Note**
      > For best practice, use the **first** ouptut of the function ```population_mld()``` to generate the value ```alpha```.
...
```
 ...
 - **output:**
   - ```maf```: a floating-point number that encodes a (stochastic) minor allele frequency $q$ for the genotype at position $j$. 
      > **Note**
      > The number ```maf``` is typically returned by the second output of a stochastic function $t \mapsto p \mapsto (\ell,q)$ generated by the process ```linkage_disequilibrium(alpha,beta,gamma,strength)```;
      >
   ...
   - ```ld```: a floating-point number $\ell$ measuring the amount of linkage disequilibrium between the two variants at the positions $i$ and $j$.
      > **Note**
      > The number ```ld``` is typically returned by the first output of the stochastic function $t \mapsto p \mapsto (\ell,q)$ that would be generated by the process ```linkage_disequilibrium(alpha,beta,gamma,strength)```;
      >
...
```python
def genmatrix(blocks: list[float],
              strength: float,
              population: float,
              Npop: int
...
```
 ...
 - **output:**
   - ```matrix```: a list $w = [w_1,w_2,\dots,w_M]$ whose elements $w_j$ are lists $[w_{j,0},w_{j,1},\dots,w_{j,N}]$ such that
       - the coefficient $w_{j,0}$ is a genetic position, which is generated from applying the process ```SNP_distribution()``` on one of the lengths contained in the list ```blocks```. Because of this, the integer $M$, which bounds the index $j$, will always be equal to the sum ```sum(blocks)+1```;
...
```
 ...
 - **inputs:**
   ...
   - ```alpha```: a bottleneck parameter $\alpha$ characterizing the amount of rare mutations in a population;
      > **Note**
      > For best practice, this parameter should be the parameter $\alpha$ used with the processes ```initiate_block()```, ```continue_block()``` or that returned by the process ```genmatrix()```.
      >
   - ```beta```: an amplitude $\beta$ for the decay behavior of the standard LD measure $r^2$;
      > **Note**
      > For best practice, this parameter should be the parameter $\beta$ used with the processes ```initiate_block()```, ```continue_block()``` or that returned by the process ```genmatrix()```.
      >
   - ```gamma```: a halfwidth $\gamma$ for the decay behavior of the standard LD measure $r^2$;
      > **Note**
      > For best practice, this parameter should be the parameter $\gamma$ used with the processes ```initiate_block()```, ```continue_block()``` or that returned by the process ```genmatrix()```.
      >
...
## LD Analytics
The processes contained in this section can be used to visualize LD-related information from the data generated by the functions presented in the [Data Generation](#data-generation) section.
...
```
 ...
 - **output:**
   ...
   - ```dist```: a list of floating-point numbers $d$ whose $i$-th coefficients $d[i]$ is the average correlation scores between two variants separated by a distance of around $i$ kilobases. 
      > **Note**
      > The first coefficient $d[0]$ is typically a residual from variants whose distance is strictly between $0$ and $1$. This is because the correlation score between two variants that are exactly $0$ kilobases apart is undefined. As a result, the first coefficient $d[0]$ is not representative of a proper distance of $0$ kilobases and should not be included in your analysis.
      >
...
```
 ...
 - **output:**
   ...
   - ```dist```: a list of floating-point numbers $d$ whose $i$-th coefficients $d[i]$ is the average LD $r^2$ scores between two variants separated by a distance of around $i$ kilobases. 
      > **Note**
      > The first coefficient $d[0]$ is typically a residual from variants whose distance is strictly between $0$ and $1$. This is because the LD $r^2$ score between two variants that are exactly $0$ kilobases apart is undefined. As a result, the first coefficient $d[0]$ is not representative of a proper distance of $0$ kilobases and should not be included in your analysis.
      >
