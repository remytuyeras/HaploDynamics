...
The generation of each locus in a VCF file tend to be linear in the parameter ```Npop```. For example, the generation of 1 SNP takes around 0.7 seconds when ```Npop=100000``` (this may vary depending on your machine specifications). Note that some generations can take much less time than the estimated time complexity (shown below).
